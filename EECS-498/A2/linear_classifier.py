...
def svm_loss_naive(
    ...
    # loss += reg * torch.sum(W * W)/(W.shape[0]*W.shape[0])
...
def softmax_loss_vectorized(
    ...
    loss = -torch.sum(torch.log(margin[torch.arange(num_train),y]))
    
    margin[torch.arange(num_train),y] -= 1
    dW = X.t().mm(margin)
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-2, 1e-1, 5e-1]
    regularization_strengths = [5e-5, 1e-4, 5e-4]
    # The best combination is 1e-1 + 1e-4 whose acc is 39.91%
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
