...
def nn_forward_backward(
    ...
    margin = torch.exp(scores) / torch.sum(torch.exp(scores),dim = 1).view(-1,1)
    loss = -torch.sum(torch.log(margin[torch.arange(N),y]))
    loss /= N
    loss += reg * torch.sum(W2 * W2) + reg * torch.sum(W1 *W1)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_get_search_params():
    ...
    learning_rate_decays = [1.0, 0.97, 0.95,0.93]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
      ...
      for lr in learning_rates:
        for hs in hidden_sizes:
          for lrd in learning_rate_decays: 
            ...
            print('train with hyperparameters: leaning_rate is:{}, hidden_size is:{},.\
             regularization_strength is:{}, learning_rate_decacy is:{}'.format(lr,hs,reg,lrd))
    ...
    print('hyperparameters with best model: leaning_rate is:{}, hidden_size is:{},.\
             regularization_strength is:{}, learning_rate_decacy is:{}'.format(lr,hs,reg,lrd))
