...
class TwoLayerNet(object):
    ...
    will interact with a separate Solver object that is responsible for running optimization.
    ...
    def loss(self, X, y=None):
        ...
        dx1, grads['W2'], grads['b2'] = Linear.backward(dout,cache2)
        grads['W2'] += 2 * self.reg *self.params['W2']
        dx2, grads['W1'], grads['b1'] = Linear_ReLU.backward(dx1,cache1)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
          h, cache1[i] = Linear_ReLU.forward(h, self.params[f'W{i}'], self.params[f'b{i}'])
          if self.use_dropout:
            h, cache2[i] = Dropout.forward(h, self.dropout_param)
        ...
        for i in range(self.num_layers - 1, 0, -1):
          if self.use_dropout:
            dh = Dropout.backward(dh, cache2[i])
          dh, grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(dh, cache1[i])
...
def get_three_layer_network_params():
    ...
    # Replace "pass" statement with your code
    weight_scale = 1e0   # Experiment with this!
    learning_rate = 1e-02  # Experiment with this!
    ################################################################
    #                             END OF YOUR CODE                 #
    ################################################################
...
def rmsprop(w, dw, config=None):
    ...
    next_w = w - (config['learning_rate'] * dw/torch.sqrt(config['epsilon']+config['cache'])) 
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def adam(w, dw, config=None):
    ...
    next_w = w - m_unbias * config['learning_rate']/(torch.sqrt(v_unbias)+config['epsilon'])
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
