...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        out = torch.zeros(N, F, H0, W0).to(x)
        
        
        # for n in range(N):
        #   for f in range(F):
        #     vec_w = w[f].reshape(-1)
        #     for h in range(0, H + 2 * pad - HH + 1, stride):
        #       for m in range(0, W + 2 * pad - WW + 1, stride):
        #         X = x[n, :, h:h+HH, m:m+WW].reshape(-1)
        #         out[n][f][h//stride][m//stride] =vec_w.dot(X)+b[f] 

        for n in range(N):
          for f in range(F):
            for h in range(H0):
              for m in range(W0):
                out[n,f,h,m] = (x[n,:,h*stride:h*stride+HH,m*stride:m*stride+WW] *w[f]).sum() + b[f]                         
        # #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
          ...
          for f in range(F):
            for h in range(H_out):
              for m in range(W_out):
                dx[n, :, h*stride:h*stride+HH, m*stride:m*stride+WW] += w[f] * dout[n, f, h, m]
                dw[f] += x[n, :, h*stride:h*stride+HH, m*stride:m*stride+WW] * dout[n, f, h, m]
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        N, C, H, W = x.shape
        H0 = 1 + (H - pool_height) // stride
        W0 = 1 + (W - pool_width) // stride
        out = torch.zeros(N,C,H0,W0).to(x)
        for n in range(N):
          for c in range(C):
            for h in range(H0):
              for w in range(W0):
    ...
    def backward(dout, cache):
        ...
        for n in range(N):
          for c in range(C):
            for h in range(H0):
              for w in range(W0):
                mask = x[n,c,h*stride:h*stride+pool_height,w*stride:w*stride+pool_width] == torch.max(x[n,c,h*stride:h*stride+pool_height,w*stride:w*stride+pool_width])
...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dh3, grads['W3'],grads['b3'] = Linear.backward(dout,cache3)
        grads['W3'] += 2 * self.reg * W3  
        dh2, grads['W2'],grads['b2'] = Linear_ReLU.backward(dh3,cache2)
        grads['W2'] += 2 * self.reg * W2
        dh1, grads['W1'],grads['b1'] = Conv_ReLU_Pool.backward(dh2,cache1)
...
class DeepConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
          ...
          if self.batchnorm:
            ...
            if i in self.max_pools:
              X, cache[i] = Conv_BatchNorm_ReLU_Pool.forward(X,w,b,gamma,beta,conv_param,self.bn_params[i],pool_param)
            else:
              X, cache[i] = Conv_BatchNorm_ReLU.forward(X,w,b,gamma,beta,conv_param,self.bn_params[i])
          else:
            if i in self.max_pools:
              X, cache[i] = Conv_ReLU_Pool.forward(X,w,b,conv_param,pool_param)
            else:
              X, cache[i] = Conv_ReLU.forward(X,w,b,conv_param)
        w = self.params[f'W{self.num_layers}']
        b = self.params[f'b{self.num_layers}']
        scores, cache[self.num_layers-1] = Linear.forward(X,w,b)
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        dh, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}']= Linear.backward(dout,cache[self.num_layers-1])
        grads[f'W{self.num_layers}'] += 2*self.reg*self.params[f'W{self.num_layers}']
        for i in range(self.num_layers-1,0,-1):
          if self.batchnorm:
            if i-1 in self.max_pools:
              dh,grads[f'W{i}'], grads[f'b{i}'], grads[f'gamma{i}'], grads[f'beta{i}'] = Conv_BatchNorm_ReLU_Pool.backward(dh,cache[i-1])
            else:
              dh,grads[f'W{i}'], grads[f'b{i}'], grads[f'gamma{i}'], grads[f'beta{i}'] = Conv_BatchNorm_ReLU.backward(dh,cache[i-1])
          else:
            if i-1 in self.max_pools:
...
class BatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        if mode == 'train':
            dxnorm = dout*gamma
            ddsig = torch.sum(dxnorm * x_mean, dim=0) * (-0.5/(sqrt_var**3))
            dmean = torch.sum(dxnorm, dim=0) * (-1/sqrt_var) - ddsig * torch.sum(2*x_mean)/N
            dx = dxnorm/sqrt_var + ddsig*2*x_mean/N + dmean/N
    ...
    def backward_alt(dout, cache):
        ...
        if mode == 'train':
            dxnorm = dout * gamma
            dx = dxnorm / sqrt_var - x_mean * torch.sum(dxnorm * x_mean, dim=0)/N/(sqrt_var**3) - torch.sum(dxnorm, dim=0) / N / sqrt_var
            dgamma = torch.sum(dout * x_norm, dim=0)
            dbeta = torch.sum(dout, dim=0)
        elif mode == 'test':
            dx = torch.sum(dout * gamma, dim=0)
