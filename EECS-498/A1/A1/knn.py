...
class KnnClassifier:
  def __init__(self, x_train, y_train):
    ...
    #self.y_train = y_train..contiguous what's the differents? results are same in this section
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      x_train = torch.cat(x_train_folds[:i] + x_train_folds[i + 1:]) #x_train中去掉第(i+1)/5部分元素
